...
//     try {
//       const demoLocations = locations; // Sử dụng dữ liệu demo từ file
//       // Hoặc lấy từ database: const demoLocations = await DiaDiemModel.find().lean();
...
//       // In ma trận khoảng cách để kiểm tra
//       console.log("Ma trận khoảng cách:");
//       console.table(distanceMatrix.map((row) => row.map((d) => d.toFixed(2))));
...
//       // Tính khoảng cách route ban đầu
...
//       const ga = new GeneticAlgorithm(demoLocations, distanceMatrix);
//       const gaRoute = ga.run(shuffledRoute);
//       const gaDistance = calculateRouteDistance(gaRoute, distanceMatrix);
//       console.timeEnd("Genetic Algorithm");

//       // Chạy Ant Colony Optimization
//       console.time("Ant Colony Optimization");
//       const aco = new AntColonyOptimization(demoLocations, distanceMatrix);
//       const acoRoute = aco.run(gaRoute); // Sử dụng kết quả từ GA làm đầu vào
...
export const optimizationController = {
  optimizeRoute: asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      // Tìm index của điểm bắt đầu và kết thúc
      ...
      // Các điểm trung gian (không bao gồm điểm đầu và cuối)
      const intermediatePoints = locationsToUse
        .map((_, idx) => idx)
        .filter((idx) => idx !== startIdx && idx !== endIdx);

      // Tối ưu hóa bằng GA trước
      ...
      // Tính toán kết quả chỉ với các điểm trong tuyến đường tối ưu
      let totalDistance = 0;
      const stops = optimizedRoute.map((index: number) => {
